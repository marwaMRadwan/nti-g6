...
createNewElement = function(parent, eleName, classes, attributes, text=''){
    ...
    attributesKeys.forEach(attr=>{
        element.setAttribute(attr, attributes[attr]) //src, attributeds[src]
    })
    return element
}
img = document.querySelector('#images')
images.forEach(element => {
    div = createNewElement(img, 'div','col-4 p-3','')
    innerDiv = createNewElement(div, 'div','border border-danger border-3','')
    img1 = createNewElement(innerDiv, 'img','img-fluid',{src:element.url})
})
